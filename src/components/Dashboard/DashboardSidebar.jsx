      ...
      >
        ...
        <div className="flex-1 overflow-y-auto flex flex-col">
          ...
          <nav className="px-3 py-3">
            ...
            {tabs.length === 0 ? (
              <p className="text-xs text-gray-400 px-2 py-1">No tabs configured.</p>
            ) : (
              tabs.map((tab) => (
                <button
                  key={tab}
                ...
                >
                  <span className={[
                    ...
                    activeTab === tab ? 'bg-brand-500' : 'bg-gray-300',
                  ].join(' ')} />
          ...
          <div className="px-4 py-3">
            <button
              onClick={onRefresh}
              disabled={loading}
              className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
            >
              <svg className={`w-4 h-4 shrink-0 ${loading ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="border-t border-gray-100 px-3 py-3 space-y-0.5 shrink-0">
          {editMode && (
            <button
              onClick={onAddWidget}
              className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-sm font-medium text-brand-600 hover:bg-brand-50 transition-colors"
            >
          ...
          <button
            onClick={onSettings}
            className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors"
          >
            <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button
            onClick={onHome}
            className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-sm text-gray-500 hover:bg-gray-50 transition-colors"
          >
            <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
